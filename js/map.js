...
function map(loadmap) {
	...
	//draw map
	   ...
	   if (this.GAMESTATE == 3) {
           this.monsterAnimate++;
           if (this.monsterAnimate == 1 || this.monsterAnimate == 7) {//two movements
                //up = 0, right = 1, down = 2, left = 3
                //moving monsters.
               anyMove = false;
                for(i = 0; i < this.monsters.length; i += 2) {
                    if (this.blockedMonster[i/2] > 0) continue;
                    moved = false;
                    _x1 = this.wappoX; _y1 = this.wappoY;
                    _x2 = this.monsters[i]; _y2 = this.monsters[i+1]; currentD = this.distance(_x1,_y1,_x2,_y2);
                    //direita
                    ...
                    if (moved) {
                        anyMove = true;
                        if (this.cells[7*this.monsters[i+1] + this.monsters[i]] & 64) {
                            this.blockedMonster[i/2] = 4;
                        }    
                    }
                }
               if (!anyMove) { this.GAMESTATE=2; this.monsterAnimate = 0; }
           }
           if (this.monsterAnimate == 12) { this.GAMESTATE=2; this.monsterAnimate = 0; }
       }
       if (this.GAMESTATE != 3 || this.monsterAnimate == 5 || this.monsterAnimate == 6) {
           //draw monsters
           for(i = 0; i < this.monsters.length; i += 2) {
